...
function displayLocal(data) {
	...
	$('#localImg').attr('src', portadasURL + local.portada);
	...
	if (local.web) {
        //class="ui-link" rel="external" target="_blank" 
		$('#linkList').append('<li><a href="' + local.web + '" rel="external">' +
			'<img src="' + iconsURL + 'web3.png"/>' +
			'<h3>Web</h3>');
	}

	if (local.facebook) {
		$('#linkList').append('<li><a href="' + local.facebook + '" rel="external">' +
			'<img src="' + iconsURL + 'facebook.png"/>' +
			'<h3>Facebook</h3>');
	}

	if (local.twitter) {
		$('#linkList').append('<li><a href="' + local.twitter + '" rel="external">' +
			'<img src="' + iconsURL + 'twitter.png"/>' +
			'<h3>Twitter</h3>');
	}
